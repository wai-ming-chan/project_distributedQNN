...
#---------------------------------------------------
# Optimization (Train the D-VQA model)
#---------------------------------------------------
...
# divide the training data into num_clients with equal distribution
X_train_VQA_split = jnp.array_split(X_train_VQA, num_clients)
Y_train_VQA_split = jnp.array_split(Y_train_VQA, num_clients)
# create local weights and bias for each client
weights_local = [weights_init for _ in range(num_clients)]
bias_local = [bias_init for _ in range(num_clients)]
...
for epoch in range(num_epochs):
    ...
    for client in range(num_clients):
        X_train_VQA_client = X_train_VQA_split[client]
        Y_train_VQA_client = Y_train_VQA_split[client]

        # shuffle the training set
        shuffle_train = np.random.permutation(len(X_train_VQA_client))
        for it in range(0, len(X_train_VQA_client), batch_size):
            X_batch = X_train_VQA_client[shuffle_train[it:it+batch_size]]
            Y_batch = [Y_train_VQA_client[shuffle_train[i]] for i in range(it, it+batch_size)]

            weight_c, bias_c = opt.step(lambda w, b: cost(w, b, X_batch, Y_batch), weights_local[client], bias_local[client])
            weights_local[client] = weight_c
            bias_local[client] = bias_c
        
    # model aggregation at central server
    weights = jnp.mean(weights_local, axis=0)
    bias = jnp.mean(bias_local, axis=0)

    # broadcast the global model to all clients
    weights_local = [weights.copy() for _ in range(num_clients)]
    bias_local = [bias.copy() for _ in range(num_clients)]
